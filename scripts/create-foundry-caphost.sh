...
usage() {
  cat << EOF
Usage: $0 --subscription-id <id> --resource-group <rg> --account-name <name> --caphost-name <name> --subnet-resource-id <id>
EOF
  exit 1
}
...
log_info "Creating capability host: ${CAPHOST_NAME}"
...
if [ "$STATUS" = "Succeeded" ]; then
  log_success "Capability host creation completed"
else
  log_error "Capability host creation failed with status: $STATUS"
  exit 1
fi
