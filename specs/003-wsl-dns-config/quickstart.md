...
**Verify VPN from WSL**:
```bash
# Check if Azure DNS is reachable
ping -c 1 168.63.129.16
```

**Expected**: Ping succeeds with response time < 100ms  
**If fails**: VPN not connected or routing issue
...
### Issue 1: Azure DNS Not Reachable

**Symptom**: `ping 168.63.129.16` fails
...
**Solution**:
```bash
# 1. Disconnect and reconnect Azure VPN (from Windows)
# 2. Verify VPN status in Azure VPN Client
# 3. Check VPN routing from Windows PowerShell:
ipconfig /all | findstr "168.63.129.16"
...
cat /etc/resolv.conf
# First nameserver MUST be 168.63.129.16
```
...
A: No, Azure DNS (168.63.129.16) handles public DNS queries normally. Fallback DNS (8.8.8.8) provides redundancy.
