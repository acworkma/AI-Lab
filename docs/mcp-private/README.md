...
Edit `bicep/mcp-api-private/policies/jwt-validation.xml` and replace the placeholders:

| Placeholder | Replace With |
|-------------|-------------|
| `REPLACE_WITH_TENANT_ID` | Your Entra tenant ID |
| `REPLACE_WITH_CLIENT_APP_ID` | Authorized client app ID(s) |
| `REPLACE_WITH_AUDIENCE` | API resource ID / audience |
...
#### 4. Run Power Platform VNet Setup
...
This script:
1. Verifies the Power Platform subnet exists with the correct delegation
2. Generates PowerShell commands to create an enterprise policy and link it to your PP environment
3. Optionally runs them if `pwsh` is available
...
New-SubnetInjectionEnterprisePolicy `
  ...
  -ResourceGroup "rg-ai-core" `
  -EnterprisePolicyName "pp-vnet-policy" `
  -EnterprisePolicyLocation "eastus2" `
  -VnetId "/subscriptions/<sub-id>/resourceGroups/rg-ai-core/providers/Microsoft.Network/virtualNetworks/vnet-ai-shared" `
  -SubnetName "PowerPlatformSubnet"

# Link to PP environment
New-SubnetInjection `
  -EnvironmentId "<pp-environment-id>" `
  -PolicyArmId "/subscriptions/<sub-id>/resourceGroups/rg-ai-core/providers/Microsoft.PowerPlatform/enterprisePolicies/pp-vnet-policy"
```

#### 5. Verify VNet Linkage
...
# Should resolve to a private IP (10.1.0.x), NOT a public IP
...
- Check that the client app ID is in the authorized applications list
